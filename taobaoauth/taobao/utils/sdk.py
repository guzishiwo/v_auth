...
class APIClient(object):
    ...
    def __init__(self, app_key, app_secret, redirect_uri=None, response_type='code', domain='oauth.taobao.com', version='2'):
        ...
        self.logout_url = 'https://oauth.taobao.com/logoff'
    ...
    def get_authorize_url(self, redirect_uri=None, **kw):
        ...
        try:
            params = _encode_params(client_id=self.client_id,
                                    response_type=response_type,
                                    redirect_uri=redirect,
                                    **kw)
        except Exception,e:
            logger.error( 'login encode_params(environment variables) is failed ,{0}', e )
            logger.info('TAOBAO_APP_KEY= {0}, TAOBAO_APP_SECRET ={1}, TAOBAO_CALLBACK_URL={3}',
                         self.client_id, response_type, redirect)
    ...
    def request_access_token(self, code, redirect_uri=None, grant_type='authorization_code'):
        ...
        try:
            ...
            logger.error('request taobao access token failed , possiable cause {0}'.format(e))
            logger.info('get access_token is failed {0}'.format(r))
            # taobao_logout_url = https://oauth.taobao.com/logoff?client_id=12304977&view=web
        return r
